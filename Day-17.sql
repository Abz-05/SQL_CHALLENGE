...
select * from patients where service in (select service from staff group by service having count(*)>(select avg(staff_count) from ( select count(*) as staff_count from staff group by service) as sub));
-- 2. List staff who work in services that had any week with patient satisfaction below 70.
select staff_name, role, service from staff where service in( select distinct service from services_weekly where patient_satisfaction < 70);
...
where patients_refused > 0) and service 
in( select service from services_weekly 
